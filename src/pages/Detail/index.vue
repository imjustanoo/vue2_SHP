          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSaleAttr) in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd changepirce="0" :class="{ active: item.isChecked === '1' }" v-for="item in spuSaleAttr.spuSaleAttrValueList" :key="item.id" @click="changeActive(item, spuSaleAttr.spuSaleAttrValueList)">{{ item.saleAttrValueName }}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <!-- 选择数量的输入框 -->
                <input autocomplete="off" class="itxt" v-model="BuyNumber" @change="changeBuyNumber">
                <a href="javascript:" class="plus" @click="BuyNumber++">+</a>
                <a href="javascript:" class="mins" @click="BuyNumber > 1 ? BuyNumber-- : BuyNumber = 1">-</a>
              </div>
              <div class="add">
...
<script>
  ...
  export default {
   ...
   methods:{
    //切换属性的高亮显示
     changeActive(checkItem,allItem) {
     ...
     changeBuyNumber(e){
       ...
       if (isNaN(val)||val<1) {
         ...
         this.BuyNumber = Math.floor(val)
       }
     },
      addShoppingCart(){
        this.$store.dispatch("CategoryShoppingCart", { skuid: this.$route.params.skuid, skunum: this.BuyNumber }).
        then(()=>{
